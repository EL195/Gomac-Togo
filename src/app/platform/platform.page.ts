...
export class PlatformPage implements OnInit {
 ...
 // course:any;
  view=true;
  style=["Entrée","Plat de Base","Dessert","Apperitif"];
  disponible=["Dinsponible","Indisponible"];
  food={
    nom:"",
    description:"",
...
comments:any[];
  ...
  this.userud();
  if(menu != null && menu!=""){
    this.itemsCollection = this.afs.collection('Repas', ref => ref.where('style', '==', menu).where('visible', '==', true));
    ...
    if(resto != null && resto!=""){
      this.restau=resto;
      this.itemsCollection = this.afs.collection('Repas', ref => ref.where('restaurant', '==', resto).where('visible', '==', true));
      this.items = this.itemsCollection.snapshotChanges().pipe(
        map(actions => {
          return actions.map(a => {
            const data = a.payload.doc.data();
      ...
      if(this.name != null && this.name!=""){
          ...
          this.itemDoc =this.afs.collection('Repas').doc(this.name);
          this.itemDoc.valueChanges().subscribe((data ) => {
            ...
            this.food.restaurant=data.restaurant;
            ...
            this.food.created=data.created;
              ...
              this.voi();
              this.fuis(this.food.restaurant);
          }) ;


      }
      else{
        this.itemsCollection = this.afs.collection('Repas', ref => ref.where('visible', '==', true));
        this.items = this.itemsCollection.snapshotChanges().pipe(
          map(actions => {
            return actions.map(a => {
              const data = a.payload.doc.data();
  ...
  fuis(restaurant: string) {
    console.log(this.food.restaurant);
    this.itemDol1 = this.afs.collection('Restaurant').doc(restaurant);
  ...
  {
    // alert(col);
    this.router.navigate(['/platform', { id: col }]);
  }
  voi() {
    this.ideaCollection1=this.afs.collection('CommentaireRepas', ref => ref.where('restaurant', '==', this.name))
  ...
  this.rcom=da.length;
    ...
    deleteR(){
      ...
      let objet=" Suppresion du Plat "+this.food.nom;
      let de=new Date().toUTCString();
      let message="Le plat "+this.food.nom+". a bien été supprimer le "+de+ ". Pour en savoir plus, veuillez contacter l'Administrateur";
      this.notifie.creatednotification(this.profil.id,message,objet);

      //alert("Votre Plat a été Supprimer");
      this.presentToast("Votre Plat a été Supprimer");
      this.afs.collection('Repas').doc(this.name).delete();
    ...
    send(){
      ...
      if(this.commentaire!=""){
        if(this.profil && this.profil !=null){
          this.afs.collection("CommentaireRepas").add({
            nom:this.profil.nom,
...
if(this.food.nom==''||this.food.nom==''){
 // alert("Veuillez renseigner le nom")
  ...
  this.food.restaurant=this.restau;
  this.food.created=this.profil.id;
  this.afs.collection('Repas').add(this.food);
  this.edit=true;
  this.view=true;
...
this.comman.restau=this.food.restaurant;
this.comman.idClient=this.profil.id;
this.comman.type="Repas";
...
if(da[0]){
  console.log(da[0]);
  console.log(this.food.restaurant);
  this.itemDol=this.afs.collection('Restaurant').doc(this.food.restaurant);
 ...
 this.item1.subscribe(data=>{
    ...
    let km=this.gps.calculateDistance(this.course.departlat,this.course.arrivelat,this.course.departlong,this.course.arrivelong);
  ...
  vero=false;
 // alert("Votre Commande a bien été effectuer");
 this.presentToast("Votre Commande a bien été effectuer");

...
export class HotelPage implements OnInit {
  ...
  newComplaint(){
    if(this.send = true){
      ...
      this.messages.add({
        "idreservation" : 'hotel_' + '_' + Math.random().toString(36).substr(2, 9),
       ...
       // "destination" : this.complaintForm.value.messageC,
        ...
        "ittienarire" : this.complaintForm.value.ville,
        "type" : this.complaintForm.value.type,
        "sortie" : firebase.firestore.Timestamp.fromDate(new Date(this.complaintForm.value.sortie)),
        "service" : "Hotel",
        "visible" : true
      })
      .then(res => {
      ...
      /*  let httpOptions = {
        ...
        this.http.post('https://afitra.cm/reserveonlineavion', formData, headers).then(

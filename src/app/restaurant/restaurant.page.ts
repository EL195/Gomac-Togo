 ...
 // itemDoc: AngularFirestoreDocument<any>;
  ...
  initialise() {
    this.afs.collection('Global', ref => ref.where('visible', '==', true)).snapshotChanges().pipe(
      ...
      this.engin=da[0].data["Restaurant"];
      console.log(da[0].data["Restaurant"]);
    })
  }
  ...
  ngOnInit() {
   ...
   if( this.name !=null &&  this.name!=""){
    this.affichage=true;
    console.log(this.name);
    this.itemDoc =this.afs.collection('Restaurant').doc(this.name);
    this.itemDoc.valueChanges().subscribe((data ) => {
      ...
      this.restoration.idRestaurant=data.idRestaurant;
  ...
  voi() {
  this.ideaCollection1=this.afs.collection('CommentaireRestaurant', ref => ref.where('restaurant', '==', this.name))
...
this.lcom=da.length;
  ...
  regarde(cat="") 
  {
    console.log(cat);
if(cat!=""){
  this.ideaCollection = this.afs.collection('Restaurant', ref => ref.where('categorie', '==', cat).where('visible', '==', true));
}
else{
  this.ideaCollection = this.afs.collection('Restaurant', ref => ref.where('visible', '==', true));
}
...
console.log(da);
  ...
  {
    // alert(col);
    this.router.navigate(['/restaurant', { id: col }]);
  }
  commande()
  {
    // alert(col);
    this.router.navigate(['/restaurant-add', { id: this.name }]);
  }
  deleteR(){
    
    let objet=" Suppresion du Restaurant "+this.restoration.nom;
    let de=new Date().toUTCString();
    let message="Le Restaurant "+this.restoration.nom+". a bien été supprimer le "+de+ ". Pour en savoir plus, veuillez contacter l'Administrateur";
    this.notifie.creatednotification(this.profil.id,message,objet);
    
 //  alert("Votre Restaurant a été Supprimer");
    this.presentToast("Votre Restaurant a été Supprimer");
    this.afs.collection('Restaurant').doc(this.name).update({"visible":false});
    this.router.navigate(['/restaurant']);
  }
  plat()
  {
    // alert(col);
    this.router.navigate(['/platform', { resto: this.name }]);
  }
...
if(this.restoration.nom !=""){
    ...
    console.log(this.restoration);
   this.afs.collection('Restaurant').doc(this.name).update(this.restoration);
let drt=new Date().toUTCString;
   let message=" Vous avez Modifier un Restaurant appélé "+this.restoration.nom+ " le " + drt;
   let Objet="Modification de Restaurant";
...
send(){
  ...
  if(this.commentaire!=""){
    if(this.profil && this.profil !=null){
      this.afs.collection("CommentaireRestaurant").add({
        nom:this.profil.nom,
